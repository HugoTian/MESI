...
void proc_t::advance_one_cycle() {
  ...
  switch (args.test) {
  case 0:
      // every one load rando address , check directory functionality 
      // if the directory is visible at the top level, read each loc of the directory and match it with the expected value
    if (!response.retry_p) {
